...
def newton_iteration_absolute_error(A, epsilon=1e-6, max_iterations=100):
    """
    Phương pháp lặp tựa Newton với sai số tuyệt đối: ||X_0|| * q^(2^k) / (1 - q) <= epsilon
    In ma trận X_k sau mỗi bước lặp.
    """
    n = A.shape[0]
    ...
    print(f"Ngưỡng sai số epsilon = {epsilon:.6e}\n")
    print(f"{'Bước':<8}{'Ma trận X_k':<40}{'Sai số ước lượng':<20}{'Ngưỡng epsilon':<20}")
    print("-" * 88)
    ...
    for k in range(max_iterations):
        ...
        print(f"{k:<8}{X_str:<40}{error:<20.6e}{epsilon:<20.6e}")
    ...
    print(f"\nKhông hội tụ sau {max_iterations} bước")
...
def main():
    try:
        ...
        # Thực hiện lặp Newton
        epsilon = 1e-6
        X, iterations = newton_iteration_absolute_error(A, epsilon)
